...
def support_touch_events(candles: list[Candle], feat: dict[str, list[float]]) -> dict[int, int]:
    """Causal 4H support episodes from the frozen written semantics.

    A pivot becomes available after its third right-hand bar closes. The pivot
    itself is touch 1. Concurrent level events are collapsed by max touch_n.
    """
    levels: list[dict] = []
    events: dict[int, int] = {}
    for i, bar in enumerate(candles):
        ...
        if touches_now:
            events[bar.close_time] = max(touches_now)
        ...
        if pivot_i >= 3:
            ...
            if all(pivot_low < other for other in neighborhood):
                levels.append({"price": pivot_low, "touch_n": 1, "last_touch_i": pivot_i, "active": True})
    return events
...
def generate_a(candles: list[Candle]) -> list[CandidateTrade]:
    feat = enrich(candles)
    ...
    while i < len(candles) - 1:
        ...
        if trade.entry_dt >= FORWARD_START_MS:
            trades.append(trade)
        if exit_i is None:
            break
        i = exit_i + 1
...
def generate_b(candles: list[Candle]) -> list[CandidateTrade]:
    feat = enrich(candles)
...
def generate_c(candles: list[Candle]) -> list[CandidateTrade]:
    feat = enrich(candles)
    trades: list[CandidateTrade] = []
    i = 672
    while i < len(candles) - 1:
...
def generate_d(candles: list[Candle]) -> list[CandidateTrade]:
    feat = enrich(candles)
    trades: list[CandidateTrade] = []
    i = 170
    while i < len(candles) - 1:
...
def generate_candidates(candles_15m: list[Candle], candles_1h: list[Candle], candles_4h: list[Candle]) -> list[CandidateTrade]:
    candidates = generate_a(candles_1h) + generate_b(candles_4h) + generate_c(candles_15m) + generate_d(candles_1h)
